...
import styles from './GerarPDFButton.module.css';
...
const GerarPDFButton = ({ dadosFiltrados }) => {
  const gerarPDF = () => {
    ...
    const colunas = [
      ...
      {
        header: 'Licenças Servidor',
        dataKey: 'licencasServidor',
      },
    ];
    ...
    const linhas = dadosFiltrados.map((servidor) => {
      ...
      // Processa licenças servidor
      const licencasServidor = servidor.periodos
        ? servidor.periodos
            .filter((p) => p.tipo === 'licenca-servidor')
            .map((p) => {
              const dataInicio = p.dataInicio
                ? typeof p.dataInicio === 'string'
                  ? new Date(p.dataInicio + 'T00:00:00')
            ...
            .join(', ')
        : 'Sem licenças servidor registradas';
      ...
      return {
        ...
        licencasServidor: licencasServidor || 'Sem afastamento registrado',
      };
    });
