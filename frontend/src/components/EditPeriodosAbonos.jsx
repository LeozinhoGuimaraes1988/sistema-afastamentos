  ...
  const handleAddAbono = async () => {
    if (novoAbono.data) {
      try {
        ...
        // Log da inserção individual
        const auth = getAuth();
        const user = auth.currentUser;
        await user?.getIdToken(true);

        const dataFormatada = new Date(novoAbono.data).toLocaleDateString(
          'pt-BR',
        );
        await logAlteracao(
          'abono',
          'inseriu',
          `Abono adicionado em ${dataFormatada} para ${servidorSelecionado.nome}`,
          user,
        );

        toast.success('Abono adicionado com sucesso!');
        setNovoAbono({ data: '' });
      } catch (error) {
    ...
    try {
      ...
      if (servidorDoc.exists()) {
        ...
        const dataFormatada = new Date(
          abonoParaRemover.data,
        ).toLocaleDateString('pt-BR');

        await logAlteracao(
          'abono',
          'removeu',
          `Abono removido em ${dataFormatada} para ${servidorSelecionado.nome}`,
          user,
        );
